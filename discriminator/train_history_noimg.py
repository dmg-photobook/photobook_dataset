...
def save_model(model, epoch, accuracy, loss, optimizer, args, metric, timestamp):
    ...
    file_name = 'model_history_noimg_' + metric + '_' + timestamp + '.pkl'
...
def evaluate(split_data_loader, dataset, breaking, normalize, mask, img_dim,\
             model, epoch, optimizer, args, timestamp, best_accuracy, best_loss, isValidation, weight, device):
    ...
    for ii, data in enumerate(split_data_loader):
        ...
        for b in range(image_set.shape[0]):
            ...
            for i in range(image_set[b].shape[0]):
                ...
                context_separate[b][i] = img_features #not used in this model
...
def gold_evaluate_write(split_data_loader, dataset, breaking, normalize, mask, img_dim, model, seg2ranks, id_list, device, criterion, threshold, weight):
    ...
    with open('hard_match_hist_noimg.txt', 'w') as file:

        with open('hard_miss_hist_noimg.txt', 'w') as miss_file:
...
if __name__ == '__main__':
    ...
    model = HistoryModelBlindNoTarget(len(vocab), embedding_dim, hidden_dim, img_dim).to(device)
    ...
    print('training no image history model')
    ...
    for epoch in range(epochs):
         ...
         with torch.no_grad():
             ...
             best_accuracy, best_loss, current_f1, current_loss = evaluate(val_loader, valset, breaking, normalize, mask, img_dim, \
                                                 model, epoch, optimizer, args, timestamp, best_accuracy, best_loss,
