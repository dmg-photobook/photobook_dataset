...
def evaluate(split_data_loader, dataset, breaking, normalize, mask, img_dim,\
    ...
    all_non_pad = 59453 if isValidation else 267891 # WARNING needs to be changed for different datasets
        ...
        for b in range(image_set.shape[0]):
            ...
            for i in range(image_set[b].shape[0]):
                ...
                if img_id != '0':
                    ...
                    context_sum[b] += img_features # NOT USED in the model
...
# used in the gold_eval_test_*.py codes to get the results on the test set
# input not batched
# not saving models
...
def gold_evaluate_write(split_data_loader, dataset, breaking, normalize, mask, img_dim, model, seg2ranks, device, criterion, threshold, weight):
    ...
    with open('hard_match_nohist.txt', 'w') as file:

        with open('hard_miss_nohist.txt', 'w') as miss_file:
...
if __name__ == '__main__':
    ...
    model = DiscriminatoryModelBlind(len(vocab), embedding_dim, hidden_dim, img_dim).to(device)
    ...
    print('training no history model')
    ...
    for epoch in range(epochs):
         ...
         for i, data in enumerate(training_loader):
            ...
            temp_batch_size = data['segment'].shape[0] #for the last batch, in case it has a different size

            context_sum = torch.zeros(temp_batch_size,img_dim).to(device) #not used in this model
